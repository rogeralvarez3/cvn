...
function get(parameters, callback) {
    if (!parameters.isLogin && parameters.table=='users') {
        ...
        let fields = parameters.fields ? Array.isArray(parameters.fields) ? parameters.fields.join(',') : parameters.fields : '*'
        let condition = parameters.condition ? ` where ${parameters.condition}` : ''
...
function save(parameters, callback) {
    let fields = [], values = [], fieldsAndValues = [], sql = '';
